...
function openSignInModal() {
  ...
  const btn     = document.querySelector('button[onclick="openSignInModal()"]');

  // 1) show overlay
  overlay.style.display = 'block';

  // 2) position & show modal
  modal.style.display    = 'block';
  modal.style.visibility = 'hidden'; // temporarily hide to measure
  const btnRect   = btn.getBoundingClientRect();
  const modalRect = modal.getBoundingClientRect();
  const scrollTop  = window.pageYOffset || document.documentElement.scrollTop;
  const scrollLeft = window.pageXOffset || document.documentElement.scrollLeft;

  // place modal just below button, centered
  const top  = btnRect.bottom + scrollTop + 8;
  const left = btnRect.left + scrollLeft + (btnRect.width/2) - (modalRect.width/2);
  ...
  window.addEventListener('resize', repositionModal(modal,modalRect));
}
...
function repositionModal(modal, modalRect) {
    const buttonRect = modal.getBoundingClientRect();
    const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
    const scrollLeft = window.pageXOffset || document.documentElement.scrollLeft;
...
// Fetch country codes from the Laravel route
 async function loadCountryCodes() {
        try {
            ...
            countries.forEach(({ dial_code }) => {
                const li = document.createElement("li");
                li.className = "px-3 py-2 cursor-pointer";
                li.textContent = dial_code;

                li.onclick = function () {
                    document.getElementById("countryCodeInput").textContent = dial_code;
